...
class Pokemonview extends StatefulWidget {
  const Pokemonview({Key? key, required this.pokemon, required this.index})
      : super(key: key);
...
class _PokemonviewState extends State<Pokemonview> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text(
              '${(widget.pokemon.no == null ? "" : "#${widget.pokemon.no} ")}${widget.pokemon.name}'),
        ),
        body: Card(
          child: Column(
            children: [
                  ...
                  children: [
                    ...
                    if (widget.pokemon.type2 != null)
                      Expanded(
                        flex: 1,
                        child:
                            _controller.getcontainertype(widget.pokemon.type2),
                      )
                  ],
                ),
              ),
              Expanded(
                  flex: 3,
                  child:
                      calculatabla(widget.pokemon.type1, widget.pokemon.type2))
            ],
          ),
        ),
        floatingActionButton: widget.pokemon.editable
            ? FloatingActionButton(
                onPressed: () async {
                  await Navigator.pushNamed(context, "/fakemon/edit",
                      arguments: {
                        "objeto": _controller.fakemon[widget.index],
                        "index": widget.index
                      });
                  setState(() {});
                },
